...
const API_KEY = "beede3bb5fc88310916252b96f99062a"; // Thay API key TMDB
const MAX_PAGE = 3;
...
export default function SinglePage() {
  ...
  // Hàm fetch nhiều trang cho từng loại phim
  ...
  // Load 3 loại phim phổ biến, top rated, sắp chiếu
  useEffect(() => {
    setLoading(true);
    Promise.all([
      fetchSeriesType(
        `https://api.themoviedb.org/3/movie/popular?api_key=${API_KEY}&language=vi-VN`,
        setPopular
      ),
      fetchSeriesType(
        `https://api.themoviedb.org/3/movie/top_rated?api_key=${API_KEY}&language=vi-VN`,
        setTopRated
      ),
      fetchSeriesType(
  ...
  const uniqueUpcoming = useMemo(() => {
    return Array.from(new Map(markSource(upcoming).map(m => [m.id, m])).values());
  }, [upcoming, genres]);

  // Hero banner lấy phim phổ biến đầu tiên
  const featuredMovies = useMemo(() => uniquePopular.slice(0, 6), [uniquePopular]);
  ...
  const gridPopular = uniquePopular.filter(m => !featuredIDs.has(m.id));
  ...
  return (
    <div className="bg-[#0f0f1b] min-h-screen text-white">
      ...
      {/* Slider phim lẻ phổ biến */}
      <MovieSectionSlider title="Phim Lẻ Phổ Biến" movies={gridPopular} genresList={genres} />

      {/* Slider phim lẻ Top Rated */}
      <MovieSectionSlider title="Phim Lẻ Top Rated" movies={uniqueTopRated} genresList={genres} />

      <AnimeSpotlightSection
        animeList={gridPopular.filter(m => m.genres.includes(16)).slice(0, 14)}/>

      {/* Slider phim lẻ Sắp Chiếu */}
      <MovieSectionSlider title="Phim Lẻ Sắp Chiếu" movies={uniqueUpcoming} genresList={genres} />
